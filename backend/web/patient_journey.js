...
(() => {
  ...
  function newPatientVisit(){
    const m=jModal('Register patient & open visit',`<form id="jRegOpen" class="formGrid">${input('First name','<input name="first_name" required>')}${input('Last name','<input name="last_name" required>')}${input('Sex','<select name="sex"><option>Female</option><option>Male</option><option>Unknown</option></select>')}${input('Phone','<input name="phone" placeholder="+255...">')}<label class="check"><input name="sms_operational_opt_in" type="checkbox" checked> Service SMS (appointments, results, billing, medication, follow-up)</label><label class="check"><input name="sms_marketing_opt_in" type="checkbox"> Health campaigns & hospital promotions</label>${input('Date of birth','<input name="date_of_birth" type="date">')}${input('Address','<input name="address">')}${input('Next of kin','<input name="next_of_kin">')}${input('Priority','<select name="priority"><option>NORMAL</option><option>URGENT</option><option>EMERGENCY</option></select>')}${input('Reason / presenting complaint','<textarea name="reason" rows="3"></textarea>')}<div class="modalActions span2"><button class="primary">Register & open file</button></div></form>`);
    m.querySelector('#jRegOpen').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target),o=Object.fromEntries(fd);o.sms_operational_opt_in=fd.has('sms_operational_opt_in');o.sms_marketing_opt_in=fd.has('sms_marketing_opt_in');if(!o.date_of_birth)delete o.date_of_birth;try{const v=await jApi('/api/journey/register-and-open',{method:'POST',body:JSON.stringify(o)});m.remove();jToast(`Patient ${v.patient_no} · file ${v.file_no}`);pages.journey()}catch(x){jToast(x.message)}};
  }
  ...
  async function renderPharmacy(){
    ...
    return `<div class="panel"><span class="eyebrow">Hospital pharmacy</span><h3>Prescription queue</h3><p class="muted">Confirm medicine availability before billing. If an item is unavailable, record it here and the patient can source it externally without losing the prescription.</p>${rows.length?rows.map(v=>`<div class="journeyVisitCard"><div class="journeyPanelHead"><div><b>${jEsc(v.file_no)} · ${jEsc(v.patient_name)}</b><span>${jEsc(v.patient_no)} · ${jBadge(v.stage)}</span></div><button class="linkBtn" data-jdetail="${v.id}">Open file</button></div><div class="journeyRx">${v.prescriptions.map(rx=>`<div><b>${jEsc(rx.medicine)}</b><span>${jEsc(rx.dose)} · ${jEsc(rx.frequency)} · Qty ${rx.quantity}${rx.status==='EXTERNAL'?' · source externally':` · stock ${rx.stock_quantity??0}`}</span><em>${jEsc(rx.status)}</em>${rx.status!=='DISPENSED'&&rx.status!=='EXTERNAL'?`<div class="actions"><button class="primary" data-jdispense="${rx.id}">Dispense</button><button class="ghost" data-junavailable="${rx.id}">Not available here</button></div>`:''}</div>`).join('')}</div><div class="journeyInvoice">${v.pharmacy_invoice?`Hospital payable: <b>${jMoney(v.pharmacy_invoice.amount)}</b> · ${jBadge(v.pharmacy_invoice.status)}`:'<span>No hospital pharmacy charge yet.</span>'}<div class="actions"><button class="ghost" data-jmedbill="${v.id}">Print full medicine bill</button>${!v.pharmacy_invoice?`<button class="ghost" data-jprepare="${v.id}">Prepare hospital bill</button>`:''}</div></div></div>`).join(''):'<div class="empty">No patients waiting at the hospital pharmacy.</div>'}</div>`;
  }
  ...
  async function prescriptionForm(v,parent){
    let catalog=[];try{catalog=await jApi('/api/prescription-catalog')}catch(x){jToast(x.message);return}
    const m=jModal('Prescribe medicine',`<form id="jRx" class="formGrid">${input('Medicine',`<input name="medicine" list="jMedList" required><datalist id="jMedList">${catalog.map(x=>`<option value="${jEsc(x.name)}"></option>`).join('')}</datalist>`)}${input('Hospital medicine',`<select name="inventory_item_id"><option value="">Manual / external medicine</option>${catalog.map(x=>`<option value="${x.id}" data-name="${jEsc(x.name)}">${jEsc(x.name)} · ${jMoney(x.unit_price)} · stock ${x.quantity}${x.available?'':' · unavailable'}</option>`).join('')}</select>`)}${input('Dose','<input name="dose" placeholder="e.g. 500 mg" required>')}${input('Frequency','<input name="frequency" placeholder="e.g. 3 times daily" required>')}${input('Duration','<input name="duration" placeholder="e.g. 5 days" required>')}${input('Quantity','<input name="quantity" type="number" min="1" value="1" required>')}${input('Medicine source','<select name="source"><option value="HOSPITAL">Hospital pharmacy</option><option value="EXTERNAL">External pharmacy</option></select>')}${input('Instructions','<textarea name="instructions" rows="2"></textarea>')}<div class="notice span2">Hospital stock is shown for guidance only. Saving a hospital prescription sends the visit to the Pharmacy queue immediately; Pharmacy confirms availability before billing and can redirect an unavailable item externally.</div><div class="modalActions span2"><button class="primary">Save / send prescription</button></div></form>`);
    const sel=m.querySelector('[name=inventory_item_id]');sel?.addEventListener('change',()=>{const op=sel.options[sel.selectedIndex];if(op?.dataset.name)m.querySelector('[name=medicine]').value=op.dataset.name});
    m.querySelector('#jRx').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target);const med={medicine:fd.get('medicine'),inventory_item_id:fd.get('inventory_item_id')?Number(fd.get('inventory_item_id')):null,dose:fd.get('dose'),frequency:fd.get('frequency'),duration:fd.get('duration'),quantity:Number(fd.get('quantity')),instructions:fd.get('instructions')||null};try{const source=fd.get('source');await jApi(`/api/journey/visits/${v.id}/prescriptions`,{method:'POST',body:JSON.stringify({medicines:[med],source})});m.remove();parent.remove();jToast(source==='HOSPITAL'?'Prescription sent to Pharmacy':'External prescription saved');pages.journey()}catch(x){jToast(x.message)}};
  }
  ...
  function bindTabActions(){
    ...
    document.querySelectorAll('[data-jmedbill]').forEach(b=>b.onclick=()=>openTokenHTML(`/api/pharmacy-documents/visits/${b.dataset.jmedbill}/medicine-bill/print`).catch(x=>jToast(x.message)));
    ...
    document.querySelectorAll('[data-junavailable]').forEach(b=>b.onclick=async()=>{const reason=prompt('Why is this medicine unavailable?','Out of stock');if(reason===null)return;try{await jApi(`/api/prescriptions/${b.dataset.junavailable}/unavailable`,{method:'PATCH',body:JSON.stringify({reason:reason||'Out of stock'})});jToast('Medicine marked unavailable; patient can source it externally');pages.journey()}catch(x){jToast(x.message)}});
