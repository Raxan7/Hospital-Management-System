...
def _open_visit(db: Session, patient: Patient, reason: str | None, priority: str, user: User) -> VisitFile:
    ...
    enc = Encounter(
        hospital_id=user.hospital_id, patient_id=patient.id, encounter_type="OPD",
    ...
    if settings.consultation_fee > 0:
        ...
        db.add(inv)
        db.flush()
    needs_payment = settings.require_consultation_payment and bool(inv and inv.amount > 0)
    ...
    v = VisitFile(
        ...
        priority=priority.upper(), reason=reason, consultation_invoice_id=inv.id if inv else None,
        opened_by=user.id,
    )
    db.add(v)
    db.flush()
...
def journey_after_dispense(db: Session, prescription: Prescription, user: User | None = None):
    ...
    paid_ok = (not _settings(db, v.hospital_id).require_pharmacy_payment) or (invoice and invoice.status == "PAID")
...
def open_visit(data: VisitOpenIn, user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    v = _open_visit(db, p, data.reason, data.priority, user)
...
def billing_queue(user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    out = []
    for v in rows:
        for kind, invoice_id in (("CONSULTATION", v.consultation_invoice_id), ("PHARMACY", v.pharmacy_invoice_id)):
...
def set_outcome(visit_id: int, data: OutcomeIn, user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    if outcome == "ADMIT":
        ...
        if choice in {"EXTERNAL", "NONE"}:
            ...
            paid_ok = (not _settings(db, v.hospital_id).require_pharmacy_payment) or (inv and inv.status == "PAID")
...
def prepare_pharmacy_bill(visit_id: int, user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    _event(db, v, "PHARMACY_INVOICE_CREATED", user.id, details={"invoice_id": inv.id, "amount": inv.amount})
